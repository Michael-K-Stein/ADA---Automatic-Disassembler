...
int IsSegment(unsigned char * opCode, char * opCMDOutput) {
    ...
    if (opCode[0] == 0x0F) { 
		return 0; } /// This is a two byte op-code

    sprintf(type, 5, "%s", (opCode[0] % 2) ? &"POP\0" : &"PUSH" );
    ...
    sprintf(opCMDOutput, 32, "%s %s", type, seg);
...
int IsSpecial(unsigned char * opCode, char * opCMDOutput, bool size_override) {
    ...
    if (ret) {
        if (_8bit) {
            sprintf(opCMDOutput, 32, "%s AL", operationType);
        } else {
            sprintf(opCMDOutput, 32, "%s %s", operationType, size_override ? &"AX\0\0" : &"EAX\0");
        }
    }
...
char JCCCMD[65] = "JO\0\0JNO\0JB\0\0JNB\0JE\0\0JNE\0JBE\0JA\0\0JS\0\0JNS\0JPE\0JPO\0JL\0\0JGE\0JLE\0JG\0\0";
int IsJCC(unsigned char * opCode, char * opCMDOutput, bool size_override) {
    ...
    if (opCode[0] == 0x0F && IsBetween(opCode[1], 0x80, 0x8F)) { 
		memcpy(opCMDOutput, JCCCMD + ((int)(opCode[1] - 0x80) * 4), 3); 
		return 1; }
...
int IsTwoByteOpCode(unsigned char * opCode, char * opCMDOutput) {
	if (opCode[0] == 0x0F && !IsBetween(opCode[1], 0x80, 0x8F)) { return 1;  }
	return 0;
}
...
int Disassemble(unsigned char * opCodes, char * output, bool _32bit, int TOTAL_FILE_OFFSET, EXTRA extra) {
    ...
    int rm_len_off = generateRM(opCodes + OpCodeOffset, rm, prefixOut.Address_Size_Override, prefixOut.Operand_Size_Override);
	...
	if (IsRegular(opCodes + OpCodeOffset,opCMD)){
        ...
        sprintf(output, 128, "%s %s", opCMD, movInstructions);
		free(movInstructions);
        OpCodeOffset+= 1 + movInstructions_len_off;
    } else if (IsOnlyIMM(opCodes + OpCodeOffset, opCMD)) {
        char * imm = (char *)calloc(16, sizeof(char)); int imm_len_off = generateIMM(opCodes + OpCodeOffset + 1, imm, prefixOut.Address_Size_Override, prefixOut.Operand_Size_Override, (IsBetween(opCodes[OpCodeOffset], 0xD4, 0xD5) || opCodes[OpCodeOffset] == 0xCD) ? 0b00 : ((opCodes[OpCodeOffset] % 4) + 1));
        sprintf(output, 128, "%s %s", opCMD, imm);
		free(imm);
        OpCodeOffset += imm_len_off + 1;
    } else if (IsSingularOpCode(opCodes + OpCodeOffset, opCMD)) {
		char * tmp = (char *)calloc(16, sizeof(char));
		SingleOpCMD(opCodes + OpCodeOffset, tmp, prefixOut.Operand_Size_Override);
        sprintf(output, 128, "%s %s", opCMD, tmp);
		free(tmp);
        OpCodeOffset++;
    } else if (IsSegment(opCodes + OpCodeOffset, opCMD)) { sprintf(output, 128, "%s", opCMD); OpCodeOffset++; }
        ...
        if (IsBetween(opCodes[0], 0x90, 0x97)) {
            ...
            sprintf(output, 128, "%s %s, %s", opCMD, prefixOut.Operand_Size_Override ? &"AX\0\0" : &"EAX\0", reg2);
            OpCodeOffset++;
            free(reg2);
        } else {
        ...
        sprintf(immF, 18, "[%s]", imm);
        char regs[11] = {"AL\0AX\0EAX\0"};
        char * reg2 = &regs[s ? (prefixOut.Operand_Size_Override ? 3 : 6 ) : 0];
        sprintf(output, 128, "%s %s, %s", opCMD, d ? immF : reg2, d ? reg2 : immF);
		free(imm); free(immF);
        OpCodeOffset += 1 + imm_len_off;
    } else if (IsJMPCall(opCodes + OpCodeOffset, opCMD)) {
        char * disp = (char *)calloc(64, sizeof(char));
        OpCodeOffset += generateJMPCall(opCodes + OpCodeOffset, disp, prefixOut.Operand_Size_Override);
        sprintf(output, 128, "%s %s", opCMD, disp);
		free(disp);
        ...
        char * imm8 = (char *)calloc(16, sizeof(char)); int imm8_len_off = generateIMM(opCodes + OpCodeOffset + 1, imm8, false, false, 0b00);
        char * reg2 = (char *)calloc(4, sizeof(char)); if (s) { memcpy(reg2, prefixOut.Operand_Size_Override ? &"AX\0" : &"EAX", 3); } else { memcpy(reg2, &"AL", 2); }
        sprintf(output, 128, "%s %s, %s", opCMD, d ? imm8 : reg2, d ? reg : imm8);
		free(imm8); free(reg2);
        ...
        if (opCodes[OpCodeOffset] == 0x69 || opCodes[OpCodeOffset] == 0x6B) { /// IMUL
            ...
            sprintf(output, 128, "%s %s, %s, %s", opCMD, reg, rm, imm);
            OpCodeOffset+=imm_len_off+rm_len_off + 1;
            free(imm);
        } else if (opCodes[OpCodeOffset] == 0x63) { /// ARPL r/m16, r16 --- Always 16bit
            char * reg2 = (char *)calloc(4, sizeof(char));      reg_len_off = getRegisterName( ( opCodes[OpCodeOffset + 1] % 64 ) >> 3, true, true, reg2  );
            char * rm2 = (char *)calloc(64, sizeof(char));      rm_len_off = generateRM(opCodes + OpCodeOffset, rm2, prefixOut.Address_Size_Override, true);
            sprintf(output, 128, "ARPL %s, %s", rm2, reg2);
			free(reg2); free(rm2);
            OpCodeOffset+= 1 + rm_len_off;
        } else if (opCodes[OpCodeOffset] == 0x8D) { /// LEA r16/32, m
            char * reg2 = (char *)calloc(4, sizeof(char));      reg_len_off = getRegisterName( ( opCodes[OpCodeOffset + 1] % 64 ) >> 3, true, prefixOut.Operand_Size_Override, reg2  );
            char * rm2 = (char *)calloc(64, sizeof(char));      rm_len_off = generateRM(opCodes + OpCodeOffset, rm2, prefixOut.Address_Size_Override, prefixOut.Operand_Size_Override);
            sprintf(output, 128, "LEA %s, %s", reg2, rm2);
			free(reg2); free(rm2);
            OpCodeOffset+= 1 + rm_len_off;
        } else if (opCodes[OpCodeOffset] == 0x8F) {
            sprintf(output, 128, "POP %s", rm);
            OpCodeOffset += 1 + rm_len_off;
        } else if (opCodes[OpCodeOffset] == 0xC8) { /// ENTER
            char * imm16 = (char *)calloc(16, sizeof(char));
            char * imm8 = (char *)calloc(16, sizeof(char));
            ...
            sprintf(output, 128, "ENTER %s, %s", imm16, imm8);
			free(imm16); free(imm8);
            ...
            char * imm16 = (char *)calloc(16, sizeof(char));
            int imm16_len_off = generateIMM(opCodes + OpCodeOffset + 1, imm16, true, true, 0b01);
            sprintf(output, 128, "%s %s", opCMD, imm16);
			free(imm16);
