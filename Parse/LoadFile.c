...
int LoadFile_PE(FILE * pFile, Portable_Executable * PE) {
    ...
    for (int secInd = 0; secInd < getNumberOfSections(PE); secInd++) {
        ...
        if (b16to10_4Bytes(ptrData) == 0) {
            ...
            unsigned char sectionDataBuffer[SizeOfRawData];

            fseek(pFile, AddressToRawData, SEEK_SET);

            if (fread(&sectionDataBuffer, 1, sizeof(sectionDataBuffer), pFile)) {
                memcpy(PE->section_tables[secInd].section_data->data, &sectionDataBuffer, sizeof(sectionDataBuffer));
            }
...
Portable_Executable LoadFile(char * fPath) {
    ...
    ptrPE = (Portable_Executable *)malloc(getFileSize("2.exe") * sizeof(unsigned char));
